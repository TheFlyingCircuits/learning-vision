...
def initCamera():
    ...
    camera.set(cv.CAP_PROP_FPS, 120) # 120 fps not working on color camera?
...
def main():
    ...
    networkTable = networkTables.getTable("hsvThreshold")
...
def detectNote(originalImage):
    ...
    greyscale = cv.GaussianBlur(greyscale, (0, 0), sigmaX=1.0)

    # color detection
    hsv = cv.cvtColor(originalImage, cv.COLOR_BGR2HSV)
    ...
    isOrange = (hue <= hMax) & (hue >= hMin) # 10 <= h <= 40
    isSaturated = (saturation >= sMin) & (saturation <= sMax) # 0 <= s <= 255
    isBright = (value <= vMax) & (value >= vMin) # 200 <= v <= 255

    isRing = (isOrange & isSaturated & isBright).reshape(height, width, 1)

    output = np.where(isRing, originalImage, 0)
    return output
    ...
    edges = gradMags > np.average(gradMags)

    gradMags = np.where(edges, 1.0, 0)

    return getVectorFieldViz(gradMags, gradAngles)
